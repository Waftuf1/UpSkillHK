...
// Support: Google Gemini > OpenRouter > OpenAI > MiniMax > Bedrock (first available wins)
...
// Gemini: gemini-2.0-flash | OpenRouter: gpt-4o | OpenAI: gpt-4o | MiniMax: MiniMax-M2.5 | Bedrock: Claude
...
function getStandardOpenAIClient(): OpenAI | null {
  if (geminiKey) return new OpenAI({ apiKey: geminiKey, baseURL: 'https://generativelanguage.googleapis.com/v1beta/openai' });
  if (openRouterKey) return new OpenAI({ apiKey: openRouterKey, baseURL: 'https://openrouter.ai/api/v1' });
  if (openaiKey) return new OpenAI({ apiKey: openaiKey });
  if (bedrockKey) return new OpenAI({ apiKey: bedrockKey, baseURL: `https://bedrock-runtime.${bedrockRegion}.amazonaws.com/openai/v1` });
  return null;
}
  ...
  chat: {
    completions: {
      create: async (params: Parameters<OpenAI['chat']['completions']['create']>[0]) => {
        // Priority: Gemini > OpenRouter > OpenAI > MiniMax > Bedrock
        if (standardOpenai) {
