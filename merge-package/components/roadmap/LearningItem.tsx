...
const RESOURCE_ICONS: Record<LearningResource['type'], string> = {
  video: '🎥',
  article: '📄',
  course: '📚',
  tool: '🛠️',
...
function ResourceBlock({ resource }: { resource: LearningResource }) {
  ...
  const content = (
    <div className="flex gap-3 p-3 bg-white rounded-lg border border-slate-100 items-start">
      ...
      {resource.url && (
        <span className="text-xs text-amber-600 font-medium flex-shrink-0">Open ↗</span>
      )}
    </div>
  );
  if (resource.url) {
    return (
      <a
        ...
        className="block hover:border-amber-300 rounded-lg transition-colors border border-transparent"
      >
        {content}
      </a>
    );
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-2 mt-2 items-center">
            ...
            <span className="px-2 py-0.5 bg-amber-100 text-amber-800 rounded text-xs">{task.skillTargeted}</span>
            ...
            {hasDetails && (
              <button
                type="button"
                onClick={() => { showDetails ? playCollapse() : playExpand(); setShowDetails(!showDetails); }}
                className="text-xs font-medium text-amber-600 hover:text-amber-800 ml-1"
              >
